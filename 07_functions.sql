...
DECLARE
    is_accepted BOOLEAN;
BEGIN
    SELECT 'ACCEPTED' = ANY (src_lpodatas.fct_c_get_committees_validation_status(_committees_validation))
    INTO is_accepted;
    RETURN is_accepted;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION src_lpodatas.fct_c_check_hidden_rules(
    _cd_nom INT, _date_min TIMESTAMP, _raw_bird_breed_code INT
)
RETURNS BOOL AS
$$
...
BEGIN
    SELECT CASE
               WHEN _raw_bird_breed_code = 99 THEN NULL
               WHEN _raw_bird_breed_code = 30 THEN 2
               WHEN _raw_bird_breed_code = 40 THEN 4
               WHEN _raw_bird_breed_code = 50 THEN 11
               ELSE _raw_bird_breed_code
               END
    ...
    SELECT exists (SELECT *
                   ...
                   WHERE rule.cd_nom = _cd_nom
                     AND (rule.all_time_restriction
                       OR (
                              _date_min BETWEEN
                                  make_date(extract(YEAR FROM now())::INT,
                                            rule.restriction_start_month::INT,
                       ...
                       OR (rule.restriction_atlas_min_code IS NOT NULL AND
                           (the_bird_breed_code IS NOT NULL AND
                            the_bird_breed_code >= rule.restriction_atlas_min_code))
                       ))
...
CREATE OR REPLACE FUNCTION src_lpodatas.fct_c_get_diffusion_level(
    _cd_nom INT, _date_min TIMESTAMP,
    _raw_bird_breed_code INT, _item JSONB
)
RETURNS INT AS
$$
DECLARE
...
BEGIN
    ...
    IF (SELECT exists(SELECT
        ...
        SELECT src_lpodatas.fct_c_check_hidden_rules(_cd_nom, _date_min, _raw_bird_breed_code) INTO the_hidden_by_rules;
...
--             SELECT ref_nomenclatures.get_id_nomenclature('NIV_PRECIS', '5')
--             INTO the_id_nomenclature_diffusion_level;
    END IF;
    /* sinon, on fait le check courant */
    SELECT CASE
               -- Taxons sensibles, règle dans la table src_lpodatas.t_c_rules_diffusion_level > diffusion au cas par cas
               WHEN _cd_nom IN (SELECT cd_nom FROM src_lpodatas.t_c_rules_diffusion_level)
                   THEN src_lpodatas.fct_c_get_taxon_diffusion_level(_cd_nom)
               -- Observation "cachée" automatiquement par les règles de sensibilités de visionature > diffusion précise
               WHEN the_hidden_by_rules THEN ref_nomenclatures.get_id_nomenclature('NIV_PRECIS', '5')
               -- Observation a priori cachée par l'observateur
               WHEN cast(_item #>> '{observers,0,hidden}' IS NOT NULL AS BOOL)
                   THEN ref_nomenclatures.get_id_nomenclature('NIV_PRECIS', '2')
