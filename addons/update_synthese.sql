...
SET
    unique_id_sinp                       = coalesce(cast(obs.item #>> '{observers,0,uuid}' AS UUID),
  ...
  , entity_source_pk_value               = obs.id::TEXT
  , id_dataset                           = CASE
                                               ...
                                               WHEN (obs.site IN
                                                     ('vn01', 'vn07', 'vn26', 'vn38', 'vn42', 'vn74', 'vn73',
                                                      'vn69') AND
                                                     obs.item #>> '{species,taxonomy}' = '8')
                                                   THEN src_lpodatas.fct_c_get_or_insert_dataset_from_shortname_with_af_id(
                                                       'LPO_SYMPETRUM',
                                                       NULL,
                                               ...
                                               ELSE
                                                   coalesce(
                                                           src_lpodatas.fct_c_get_dataset_from_observer_uid(obs.item #>> '{observers,0,@uid}'),
  ...
  , id_nomenclature_grp_typ              = gn_synthese.get_default_nomenclature_value('TYP_GRP'::CHARACTER VARYING)
  , id_nomenclature_obs_technique        = ref_nomenclatures.get_id_nomenclature('METH_OBS', '21')
  , id_nomenclature_bio_status           = coalesce(
        coalesce(ref_nomenclatures.fct_c_get_synonyms_nomenclature('STATUT_BIO',
                                                                   obs.item #>>
                                                                   '{observers,0,atlas_code}'),
                 ref_nomenclatures.fct_c_get_synonyms_nomenclature('STATUT_BIO', obs.item #>>
  ...
  , id_nomenclature_exist_proof          =
        CASE
            WHEN obs.item #> '{observers,0}' ? 'medias' THEN
  ...
  , id_nomenclature_valid_status         =
        ...
        CASE
            WHEN obs.item #>> '{observers,0,committees_validation,chr}' LIKE 'ACCEPTED'
  ...
  , id_nomenclature_diffusion_level      = CASE
    ...
    -- Observation "invalide" (> refused) ou en ffquestionnement (> question)
                                               WHEN obs.item #>> '{observers,0,admin_hidden_type}' IN
                                                    ('refused', 'question') THEN
  ...
  , id_nomenclature_sensitivity          = CASE
                                               WHEN cast(obs.item #>> '{observers,0,hidden}' AS BOOLEAN) THEN
  ...
  , id_nomenclature_observation_status   = CASE
                                               WHEN ((obs.item #>> '{observers,0,count}' = '0'
                                                   AND obs.item #>> '{observers,0,estimation_code}' LIKE
                                                       'EXACT_VALUE') OR
                                                     (obs.item #>> '{observers,0,atlas_code}' = '99')) THEN
  ...
  , cd_nom                               = cast(coalesce(src_lpodatas.fct_c_get_taxref_values_from_vn('cd_nom'::TEXT,
                                                                                                      cast(obs.item #>> '{species,@id}' AS INTEGER)),
                                                         gn_commons.get_default_parameter('visionature_default_cd_nom')) AS INTEGER)
  , nom_cite                             = src_lpodatas.fct_c_get_species_values_from_vn('latin_name'::TEXT,
                                                                                         cast(obs.item #>> '{species,@id}' AS INTEGER))
...
--                           4326), 2154)
  ...
  , observers                            = CASE
                                               WHEN (obs.item #> '{observers,0}') ? 'second_hand' AND
                                                    (obs.item #>> '{observers,0,second_hand}') = '1'
                                                   THEN NULL
                                               ELSE src_lpodatas.fct_c_get_role_name_from_visionature_uid(
                                                           obs.item #>> '{observers,0,@uid}', TRUE) END
  , determiner                           = CASE
                                               WHEN (obs.item #> '{observers,0}') ? 'second_hand' AND
                                                    (obs.item #>> '{observers,0,second_hand}') = '1'
                                                   THEN NULL
  ...
  , reference_biblio                     = src_lpodatas.fct_c_get_source_url(
    ...
    FROM
        src_vn_json.observations_json obs
    WHERE
...
SET
  ...
  , taxo_group          =
        src_lpodatas.fct_c_get_taxo_group_values_from_vn('name', obs.site,
                                                         cast(obs.item #>> '{species,taxonomy}' AS INT))
  , taxo_real           = src_lpodatas.fct_c_get_taxref_values_from_vn('id_rang'::TEXT,
                                                                       cast(obs.item #>> '{species,@id}' AS INTEGER)) IN
                          ('ES', 'SSES')
  , common_name         =
        CASE
            WHEN src_lpodatas.fct_c_get_taxref_values_from_vn('nom_vern'::TEXT,
                                                              cast(obs.item #>> '{species,@id}' AS INTEGER)) IS NOT NULL
                THEN
                split_part(src_lpodatas.fct_c_get_taxref_values_from_vn('nom_vern'::TEXT,
                                                                        cast(obs.item #>> '{species,@id}' AS INTEGER)),
                           ',', 1)
            ELSE
                src_lpodatas.fct_c_get_species_values_from_vn('french_name'::TEXT,
                                                              cast(obs.item #>> '{species,@id}' AS INTEGER))
            END
  ...
  , date_year           = cast(extract(YEAR FROM
                                       to_timestamp(cast(obs.item #>> '{date,@timestamp}' AS DOUBLE PRECISION))) AS INTEGER)
  , mortality           = cast(((obs.item #>> '{observers,0,extended_info,mortality}'::TEXT[]) IS NOT NULL) AS BOOLEAN)
  ...
  , observers           = CASE
                              WHEN (obs.item #> '{observers,0}') ? 'second_hand' AND
                                  ...
                                  THEN NULL
                              ELSE src_lpodatas.fct_c_get_role_name_from_visionature_uid(
                                          obs.item #>> '{observers,0,@uid}', FALSE) END
    FROM
        gn_synthese.synthese
            JOIN src_vn_json.observations_json obs
                 ON (synthese.id_source, synthese.entity_source_pk_value) =
                    (src_lpodatas.fct_c_upsert_or_get_source_from_visionature(obs.site), obs.id::TEXT)
    WHERE
...
SELECT
  ...
  , jsonb_pretty(item)
    ...
    WHERE
        item::TEXT LIKE '%Serena-SGGA-27616%'
    LIMIT 1
;
