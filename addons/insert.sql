...
SELECT
    coalesce(
        (new.item #>> '{observers,0,uuid}')::UUID,
    ...
    CASE
        ...
        ELSE
            coalesce(
                src_lpodatas.fct_c_get_dataset_from_observer_uid(new.item #>> '{observers,0,@uid}'),
                src_lpodatas.fct_c_get_or_insert_dataset_from_shortname(
                    NULL,
                    'visionature_default_dataset',
                    'visionature_default_acquisition_framework'
    ...
    CASE
        WHEN new.item #> '{observers,0}' ? 'medias'
            THEN
    ...
    CASE
        ...
        WHEN
            (new.item #>> '{observers,0,admin_hidden}')::BOOLEAN
            OR NOT src_lpodatas.fct_c_get_committees_validation_is_accepted(
                new.item #> '{observers,0,committees_validation}'
            )
            THEN
    ...
    CASE
        ...
        WHEN (coalesce(src_lpodatas.fct_c_get_taxref_values_from_vn(
            'cd_nom'::TEXT,
            (new.item #>> '{species,@id}')::INTEGER
        ),
        gn_commons.get_default_parameter('visionature_default_cd_nom')))::INTEGER IN
        (SELECT cd_nom FROM src_lpodatas.t_c_rules_diffusion_level)
            THEN src_lpodatas.fct_c_get_taxon_diffusion_level((coalesce(
        ...
        -- Observation "cachée"
        WHEN (new.item #>> '{observers,0,hidden}' IS NOT NULL)::BOOL
            THEN
                ref_nomenclatures.get_id_nomenclature('NIV_PRECIS', '2')
        -- Observation "invalide" (> refused) ou en ffquestionnement (> question)
        WHEN new.item #>> '{observers,0,admin_hidden_type}' IN ('refused', 'question')
            THEN
    ...
    CASE
        WHEN (new.item #>> '{observers,0,hidden}')::BOOLEAN
            THEN
    ...
    CASE
        WHEN ((
            new.item #>> '{observers,0,count}' = '0'
            AND new.item #>> '{observers,0,estimation_code}' LIKE 'EXACT_VALUE'
        )
        OR (new.item #>> '{observers,0,atlas_code}' = '99'))
            THEN
    ...
    (new.item #>> '{observers,0,count}')::INTEGER,
    (new.item #>> '{observers,0,count}')::INTEGER,
    (coalesce(src_lpodatas.fct_c_get_taxref_values_from_vn(
        'cd_nom'::TEXT,
        (new.item #>> '{species,@id}')::INTEGER
    ),
    gn_commons.get_default_parameter('visionature_default_cd_nom')))::INTEGER,
    NULL,
    src_lpodatas.fct_c_get_species_values_from_vn('latin_name'::TEXT, (new.item #>> '{species,@id}')::INTEGER),
    ...
    NULL,
    (new.item #>> '{observers,0,altitude}')::INTEGER,
    (new.item #>> '{observers,0,altitude}')::INTEGER,
    public.st_setsrid(public.st_makepoint(
        (((new.item -> 'observers') -> 0) ->> 'coord_lon')::FLOAT,
        (((new.item -> 'observers') -> 0) ->> 'coord_lat')::FLOAT
    ),
    4326),
    public.st_setsrid(public.st_makepoint(
        (((new.item -> 'observers') -> 0) ->> 'coord_lon')::FLOAT,
    ...
    4326),
    to_timestamp((new.item #>> '{date,@timestamp}')::DOUBLE PRECISION),
    to_timestamp((new.item #>> '{date,@timestamp}')::DOUBLE PRECISION),
    NULL,
    CASE
        WHEN (new.item #> '{observers,0}') ? 'second_hand' AND (new.item #>> '{observers,0,second_hand}') = '1'
            THEN NULL
        ELSE src_lpodatas.fct_c_get_role_name_from_visionature_uid(new.item #>> '{observers,0,@uid}', TRUE)
    END,
    CASE
        WHEN (new.item #> '{observers,0}') ? 'second_hand' AND (new.item #>> '{observers,0,second_hand}') = '1'
            THEN NULL
...
SELECT
    ...
    src_lpodatas.fct_c_get_taxref_values_from_vn('id_rang'::TEXT, (new.item #>> '{species,@id}')::INT) IN (
        'ES', 'SSES'
    ),
    CASE
        WHEN
            ...
            THEN
                split_part(
                    src_lpodatas.fct_c_get_taxref_values_from_vn(
                        'nom_vern'::TEXT, (new.item #>> '{species,@id}')::INT
                    ),
                    ',', 1
                )
        ELSE
    ...
    END,
    encode(hmac(((new.item #>> '{observers,0,@uid}'))::TEXT, 'congi#ewie7eiX_u', 'sha1'), 'hex'),
    (new.item #>> '{observers,0,atlas_code}')::INTEGER,
    CASE
    ...
    (extract(YEAR FROM to_timestamp((new.item #>> '{date,@timestamp}')::DOUBLE PRECISION)))::INTEGER,
    (((new.item #>> '{observers,0,extended_info,mortality}'::TEXT []) IS NOT NULL))::BOOLEAN,
    new.item #>> '{observers,0,extended_info, mortality, death_cause2}',
    FALSE,
    new.item #>> '{observers,0,project_code}',
    src_lpodatas.fct_c_get_entity_from_observer_site_uid(
        ((new.item #>> '{observers,0,@uid}'))::INTEGER,
        new.site
    ),
    ...
    (new.item #>> '{place,@id}')::INTEGER,
    new.item #>> '{place,name}',
    new.id_form_universal,
    coalesce(NOT (new.item #>> '{observers,0,admin_hidden}')::BOOLEAN, TRUE),
    new.item #>> '{observers,0,hidden_comment}',
    (new.item #>> '{observers,0,hidden}' IS NOT NULL)::BOOL
FROM
    gn_synthese.synthese AS sy
INNER JOIN gn_synthese.t_sources AS so ON sy.id_source = so.id_source
INNER JOIN src_vn_json.observations_json AS new ON (name_source, entity_source_pk_value::INT) = (new.site, new.id)
