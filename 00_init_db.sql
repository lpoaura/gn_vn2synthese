...
CREATE TABLE IF NOT EXISTS src_lpodatas.t_c_synthese_extended (
	...
	(id_synthese) ON DELETE CASCADE ON UPDATE NO ACTION , id_sp_source
	INTEGER , taxo_group VARCHAR(50) , taxo_real BOOLEAN , common_name
	VARCHAR(250) , pseudo_observer_uid VARCHAR(200) , observers
	VARCHAR(200) , bird_breed_code INTEGER , breed_status VARCHAR(20) ,
	bat_breed_colo BOOLEAN , bat_is_gite BOOLEAN , bat_period VARCHAR(20) ,
	estimation_code VARCHAR(50) , date_year INTEGER , mortality BOOLEAN ,
	mortality_cause VARCHAR(250) , export_excluded BOOLEAN , project_code
	VARCHAR(250) , juridical_person VARCHAR(100) , behaviour TEXT[] ,
	geo_accuracy VARCHAR(50) , details JSONB , id_place INT , place
	VARCHAR(250) , id_form VARCHAR(20) , is_valid BOOLEAN , private_comment
	TEXT , is_hidden BOOLEAN DEFAULT FALSE
);
...
CREATE INDEX IF NOT EXISTS i_t_c_synthese_extended_id_sp_source ON
    src_lpodatas.t_c_synthese_extended (id_sp_source);

CREATE INDEX IF NOT EXISTS i_t_c_synthese_extended_taxo_group ON
    src_lpodatas.t_c_synthese_extended (taxo_group);

CREATE INDEX IF NOT EXISTS i_t_c_synthese_extended_common_name ON
    src_lpodatas.t_c_synthese_extended (common_name);

CREATE INDEX IF NOT EXISTS i_t_c_synthese_extended_id_place ON
    src_lpodatas.t_c_synthese_extended (id_place);

CREATE INDEX IF NOT EXISTS i_t_c_synthese_extended_is_valid ON
    src_lpodatas.t_c_synthese_extended (is_valid);

CREATE INDEX IF NOT EXISTS i_t_c_synthese_extended_is_hidden ON
    src_lpodatas.t_c_synthese_extended (is_hidden);

CREATE INDEX IF NOT EXISTS i_t_c_synthese_extended_bird_breed_code_txt ON
    src_lpodatas.t_c_synthese_extended (cast(bird_breed_code AS TEXT));

CREATE INDEX IF NOT EXISTS i_t_c_synthese_extended_bird_breed_code ON
    src_lpodatas.t_c_synthese_extended (bird_breed_code);

CREATE INDEX IF NOT EXISTS i_t_c_synthese_extended_project_code ON
    src_lpodatas.t_c_synthese_extended (project_code);

CREATE TABLE src_lpodatas.t_c_rules_diffusion_level (
    id_rule_diffusion_level SERIAL PRIMARY KEY , cd_nom INT REFERENCES
	taxonomie.taxref , id_nomenclature_diffusion_level INTEGER CONSTRAINT
	fk_synthese_id_nomenclature_diffusion_level REFERENCES
	ref_nomenclatures.t_nomenclatures ON UPDATE CASCADE CONSTRAINT
	check_synthese_diffusion_level CHECK
	(ref_nomenclatures.check_nomenclature_type_by_mnemonique
	(id_nomenclature_diffusion_level , cast('NIV_PRECIS' AS CHARACTER
	VARYING))) , meta_create_date TIMESTAMP DEFAULT now() ,
	meta_update_date TIMESTAMP DEFAULT now()
);

CREATE UNIQUE INDEX ON src_lpodatas.t_c_rules_diffusion_level (cd_nom ,
...
CREATE TRIGGER tri_meta_dates_change_synthese
    BEFORE INSERT OR UPDATE ON src_lpodatas.t_c_rules_diffusion_level
    FOR EACH ROW
    EXECUTE PROCEDURE fct_trg_meta_dates_change ();

INSERT INTO src_lpodatas.t_c_rules_diffusion_level (cd_nom ,
    id_nomenclature_diffusion_level)
SELECT cd_nom , ref_nomenclatures.get_id_nomenclature ('NIV_PRECIS' , '4')
