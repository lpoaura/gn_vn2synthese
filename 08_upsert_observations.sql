...
BEGIN
    ...
    SELECT uuid INTO the_unique_id_sinp_grp
	...
	-> 'observers') -> 0) ->> 'coord_lat' AS FLOAT)) , 4326) INTO
	_the_geom_4326;
	...
	('TYP_GRP'::CHARACTER VARYING) INTO the_id_nomenclature_grp_typ;
	...
	'{species,taxonomy}')::INT , NEW.item) IN ('Possible' , 'Probable'
	, 'Certain') THEN
			...
			, '1')) INTO
			the_id_nomenclature_bio_status;
    SELECT CASE WHEN NEW.item #> '{observers,0,extended_info}' ? 'mortality' THEN
    ...
    SELECT CASE WHEN NEW.item #> '{observers,0}' ? 'medias' THEN
    ...
    SELECT CASE WHEN cast(NEW.item #>> '{observers,0,hidden}' AS BOOLEAN) THEN
    ...
    SELECT CASE WHEN ((NEW.item #>> '{observers,0,count}' = '0'
	...
	('latin_name'::TEXT , the_id_sp_source) INTO the_nom_cite;
    SELECT gn_commons.get_default_parameter ('taxref_version' , NULL) INTO
	the_meta_v_taxref;
	...
	(gn_commons.get_default_parameter ('gn_local_srid' , NULL))::INT) INTO
	_the_geom_local;

    SELECT NULL INTO the_validation_comment;
    SELECT CASE WHEN (NEW.item #> '{observers,0}') ? 'second_hand'
    ...
    SELECT gn_synthese.get_default_nomenclature_value ('METH_DETERMIN') INTO
	...
	, cast(NEW.item #>> '{species,@id}' AS INTEGER)) IN ('ES'
	, 'SSES') INTO the_taxo_real;
    SELECT CASE WHEN src_lpodatas.fct_c_get_taxref_values_from_vn
    ...
    SELECT CASE WHEN (NEW.item #> '{observers,0}') ? 'atlas_code' THEN
	...
	'{date,@timestamp}' AS DOUBLE PRECISION))) AS INTEGER) INTO the_date_year;
    SELECT cast(((NEW.item #>> '{observers,0,extended_info,mortality}'::TEXT[]) IS NOT NULL) AS
	BOOLEAN) INTO the_mortality;
	...
	(NEW.item #> '{observers,0,behaviours}') INTO the_behaviour;
    ...
    RAISE DEBUG '-- % -- Populate variables > ENDING : duration %' , start_ts , (
        SELECT (clock_timestamp() - start_ts));
    IF (tg_op = 'UPDATE')
	...
	SET unique_id_sinp = coalesce(unique_id_sinp , the_unique_id_sinp) ,
	    ...
	    the_id_source , entity_source_pk_value = the_entity_source_pk_value
	    , id_dataset = the_id_dataset , id_nomenclature_geo_object_nature =
	    the_id_nomenclature_geo_object_nature , id_nomenclature_grp_typ =
		...
		the_id_nomenclature_info_geo_type , count_min = the_count_min ,
		count_max = the_count_max , cd_nom = the_cd_nom , nom_cite =
		the_nom_cite , meta_v_taxref = the_meta_v_taxref ,
		...
		the_altitude_max , the_geom_4326 = _the_geom_4326 ,
		the_geom_point = _the_geom_point , the_geom_local =
		_the_geom_local , date_min = the_date_min , date_max =
		the_date_max , validation_comment = the_validation_comment ,
		observers = the_observers , id_digitiser = the_id_digitiser ,
		id_nomenclature_determination_method =
		the_id_nomenclature_determination_method , comment_description
		= the_comments , reference_biblio = the_reference_biblio
	...
	SET id_synthese = the_id_synthese , id_sp_source = the_id_sp_source ,
	    taxo_group = the_taxo_group , taxo_real = the_taxo_real ,
	    common_name = the_common_name , pseudo_observer_uid =
	    the_pseudo_observer_uid , bird_breed_code = the_bird_breed_code ,
	    breed_status = the_breed_status , bat_breed_colo =
	    the_bat_breed_colo , bat_is_gite = the_bat_is_gite , bat_period =
	    the_bat_period , estimation_code = the_estimation_code , date_year
	    = the_date_year , mortality = the_mortality , mortality_cause =
	    the_mortality_cause , export_excluded = the_export_excluded ,
	    project_code = the_project_code , juridical_person =
	    the_juridical_person , behaviour = the_behaviour , geo_accuracy =
	    the_geo_accuracy , details = the_details , id_place = the_id_place
	    , place = the_place , id_form = the_id_form , is_valid =
	    the_is_valid , private_comment = the_private_comment , is_hidden =
...
BEGIN
	...
	the_unique_id_sinp;
    SELECT id_synthese INTO the_id_synthese
