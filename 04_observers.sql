...
DROP FUNCTION IF EXISTS src_lpodatas.fct_c_create_gnadm_observer_from_visionature (_item JSONB, _rq TEXT);

CREATE OR REPLACE FUNCTION src_lpodatas.fct_c_create_gnadm_observer_from_visionature (_item jsonb, _rq text DEFAULT 'Utilisateur VisioNature')
    RETURNS int
    AS $$
DECLARE
    theroleid int;
...
COMMENT ON FUNCTION src_lpodatas.fct_c_create_gnadm_observer_from_visionature (_item JSONB, _rq TEXT) IS 'créée ou mets à jour un observervateur à partir des entrées json VisioNature';
...
BEGIN
    PERFORM
        src_lpodatas.fct_c_create_gnadm_observer_from_visionature (NEW.item);
    RETURN new;
END;
$$;

ALTER FUNCTION src_lpodatas.fct_tri_c_vn_observers_to_geonature () OWNER TO gnadm;
